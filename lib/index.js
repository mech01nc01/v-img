...
const addPluginAttributes = (el, binding, options) => {
  ...
  let openOn;
  let sourceButton;
  let src = el.dataset.fullSrc; // eslint-disable-line prefer-destructuring
  let thumbSrc = el.src;
  let title;
  let thumbnails;
  const events = {};
  ...
  if (typeof binding.value !== 'undefined') {
    ...
    thumbSrc = binding.value.thumbSrc || thumbSrc;
  ...
  el.setAttribute('data-vue-img-thumb-src', thumbSrc);
  ...
  if (!src && !thumbSrc) console.error('v-img element missing src parameter.');
  ...
  Vue.directive('img', {
    ...
    bind(el, binding) {
      ...
      el.addEventListener(addedAttributes.openOn, () => {
        ...
        Vue.set(vm, 'thumbs', images.map(e => e.dataset.vueImgThumbSrc));
